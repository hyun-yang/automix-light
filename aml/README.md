...
먼저 준비물부터 갖춘다. `CLAUDE.md`(Claude Code 가 매 세션 자동으로 읽는 프로젝트 안내 파일)가 없으면 인터뷰 결과로 만들어 주고 — **이미 있으면 손대지 않는다** — git 저장소가 아니면 초기화한 뒤 기본 브랜치를 `main` 으로 맞춘다. 이미 git 저장소면 아무것도 바꾸지 않는다.

그다음 Claude 가 인터뷰로 지도를 그린다. 네 구역을 차례로 캐낸다.
...
마지막에 `spec.md`("완성 기준" 목록 포함)와 `task.md` 가 만들어지고, 사용자가 검토한다. (`CLAUDE.md` 가 없었다면 이때 함께 만들어진다.)
...
- ⚠️ **참고만**: CLAUDE.md 없음, git 없음, python3 없음(측정 줄이 안 남음), Langfuse 설정 문제
...
/aml:new "칸반처럼 쓰는 할 일 앱, HTML 파일 하나로"
  → git 준비(기본 브랜치 main) + CLAUDE.md 생성
