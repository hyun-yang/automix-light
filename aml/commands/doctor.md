...
4. git 저장소인가(`git rev-parse --is-inside-work-tree`)? — 아니면: "git 은 필수가 아니지만, 있으면 실수를 되돌리기 쉽습니다"
5. python3 를 쓸 수 있는가(`command -v python3`)? — 아니면: "progress.md 에 측정 줄(모델·시간·토큰·비용)이 남지 않습니다"
6. Langfuse 상태 — 셋 중 하나로 보고한다. **Langfuse 는 어떤 경우에도 막지 않는다.**
